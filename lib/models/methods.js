...
MethodsModel = function (metricsThreshold) {
  ...
  this.histogramBuilder = new HistogramBuilder('methods');
};
...
MethodsModel.prototype.processMethod = function(methodTrace) {
  ...
  // add resTime to the histogram
  this.histogramBuilder.add(dateId, methodTrace.name, methodTrace.metrics.total);
...
MethodsModel.prototype.buildPayload = function(buildDetailedInfo) {
  ...
  // collect histograms
  payload.methodHistograms = this.histogramBuilder.collectHistograms();
