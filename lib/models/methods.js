...
MethodsModel = function (metricsThreshold) {
  ...
  setInterval(function() {
    console.log(
      'SIZE_INFO', 
      _.keys(self.methodMetricsByMinute).length,
      _.keys(self.errorMap).length,
      _.keys(self.maxEventTimesForMethods).length
    );
  }, 5000);
};
...
MethodsModel.prototype.processMethod = function(method) {
  ...
  var atDate = new Date(metrics.at);
  var dateId = atDate.getHours() + ':' + atDate.getMinutes();
