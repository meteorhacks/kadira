...
PubsubModel.prototype._trackNetworkImpact = function(session, sub, event, collection, id, fields) {
  ...
  if(event != 'removed' && _.keys(fields).length > 0) {
    ...
    if(subscriptionState) {
      ...
      sub.totalDataSent += Buffer.byteLength(JSON.stringify(_.values(fields)));
...
PubsubModel.prototype._countDocuments = function() {
  var self = this;
  var countMap = {};
  for(var sessionId in Meteor.default_server.sessions) {
    var session = Meteor.default_server.sessions[sessionId];
    //count docs from normal subs
    _.each(session._namedSubs, function(sub) {
      var publication = self._getPublicationName(sub._name);
      countMap[publication] = countMap[publication] || 0;
      for(collectionName in sub._documents) {
        countMap[publication] += _.keys(sub._documents[collectionName]).length;
      }
    });
    //count docs from null subs
    _.each(session._universalSubs, function (sub) {
      var publication = self._getPublicationName(null);
      countMap[publication] = countMap[publication] || 0;
      for(collectionName in sub._documents) {
        countMap[publication] += _.keys(sub._documents[collectionName]).length;
      }
...
PubsubModel.prototype.buildPayload = function(buildDetailInfo) {
  ...
  var documentCount = this._countDocuments();
  ...
  for(var dateId in metricsByMinute) {
    for(var publication in metricsByMinute[dateId].pubs) {
      ...
      singlePubMetrics.activeSubs = subscriptionCount[publication] || 0;
      singlePubMetrics.activeDocs = documentCount[publication] || 0;
    }
