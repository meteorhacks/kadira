...
PubsubModel.prototype._getSubscriptionInfo = function() {
  ...
  _.each(totalObserverHandlers, function(value, publication) {
    avgObserverReuse[publication] = cachedObservers[publication] / totalObserverHandlers[publication];
  });
  ...
  function countObservers(sub, publication) {
    totalObserverHandlers[publication] = totalObserverHandlers[publication] || 0;
    cachedObservers[publication] = cachedObservers[publication] || 0;

    totalObserverHandlers[publication] += sub._totalObserverHandlers;
...
PubsubModel.prototype.incrementHandleCount = function(trace, isCached) {
  ...
  if(session) {
    ...
    if(sub) {
      sub._totalObserverHandlers = sub._totalObserverHandlers || 0;
      sub._cachedObservers = sub._cachedObservers || 0;
    }
  }
  // not sure, we need to do this? But I don't need to break the however
  sub = sub || {_totalObserverHandlers:0 , _cachedObservers: 0};

  publication.totalObserverHandlers++;
  sub._totalObserverHandlers++;
  if(isCached) {
