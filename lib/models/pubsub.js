...
PubsubModel = function() {
  ...
  this.histogramBuilder = new HistogramBuilder('pubsub');
}
...
PubsubModel.prototype._trackReady = function(session, sub, trace) {
  ...
  if(subscriptionState && !subscriptionState.readyTracked) {
    ...
    // Add resTime to the histogram builder
    this.histogramBuilder.add(dateId, sub._name, metrics.resTime);
  }
...
PubsubModel.prototype.buildPayload = function(buildDetailInfo) {
  ...
  payload.pubHistograms = this.histogramBuilder.collectHistograms();

  // TODO: collect from the histogram builder
