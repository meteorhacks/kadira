  ...
  function callHTTP() {
    Kadira._postData(endpoint, httpOptions, function(err, response){
      ...
      if(status < 400 || status > 600) {
        err = err || new Error("Unknown error: " + status);
        return tryAgain(err);
      }

      if(status >= 500 && status < 600) {
        if(!err) {
          err = new Error("Kadira: Unknown server error: " + status);
        }
      ...
      if(status === 429 || status === 430) {
        console.error('Kadira: Rate limit error');
      } else if(status >= 400 && status < 500) {
        console.error('Kadira: Unknown kadira agent error');
      }
