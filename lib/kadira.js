...
var logger = Npm.require('debug')('kadira:kadira');
...
Kadira.connect = function(appId, appSecret, options) {
  ...
  Kadira.messenger = new Messenger(appId, appSecret, options.endpoint);
  ...
  function sendPayload() {
    ...
    function callHTTP() {
      // buildPayload() requires a fiber.
      // That's why we are running this inside a fiber
      new Fibers(function() {
        var payload = buildPayload();
        var httpOptions = {json: payload};

        Kadira.messenger.sendRequest('POST', '/', httpOptions, processResponse);
      }).run();
  ...
  function pingToCheckAuth(callback){
    ...
    Kadira.messenger.sendRequest('POST', '/ping', httpOptions, function(err, response) {
      if(response) {
