...
Kadira.trackError = function (error) {
  // TODO: only send `Kadira.options.maxErrorsPerInterval` 
 ...
 */
Kadira.sendErrors = function (errors, force) {
  if(force) {
  ...
  if(Kadira.syncedDate.synced) {
    Kadira.sendErrors(errors, true);
  } else {
    // TODO: increase timeout to 5 secs
    setTimeout(Kadira.sendErrors.bind(this, errors, true), 1000*2);
  }
}
