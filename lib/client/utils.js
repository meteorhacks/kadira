...
getErrorStack = function (zone, callback) {
  ...
  trace.push({
    at: (new Date().getTime()),
    stack: getNormalizedStacktrace(zone.erroredStack._e.stack)
  });
  ...
  function processZone() {
    ...
    if(zone && zone.depth > 2) {
      ...
      if(zone.currentStack) {
        stack = getNormalizedStacktrace(zone.currentStack._e.stack);
      }
...
getNormalizedStacktrace = function (stack) {
  ...
  var withoutZones = _.filter(stackLines, stackFramesFilter);
  return withoutZones.join('\n');
}
