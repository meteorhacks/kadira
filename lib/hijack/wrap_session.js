...
wrapSession = function(sessionProto) {
  ...
  sessionProto.processMessage = function(msg) {
    if(Kadira.connected) {
      ...
      if(msg.msg == 'method' || msg.msg == 'sub') {
        apmInfo.trace = Kadira.tracer.start(this, msg);
        ...
        Kadira.tracer.event(apmInfo.trace, 'start', startData);
        var waitEventId = Kadira.tracer.event(apmInfo.trace, 'wait', {waitOn: waitOnMessages}, apmInfo);
  ...
  sessionProto.protocol_handlers.method = function(msg, unblock) {
    ...
    Kadira._setInfo(apmInfo);

    Kadira.tracer.eventEnd(apmInfo.trace, msg._waitEventId);
  ...
  sessionProto.send = function(msg) {
    if(msg.msg == 'result') {
      ...
      if(msg.error) {
        ...
        Kadira.tracer.event(apmInfo.trace, 'error', {error: error});
      } else {
