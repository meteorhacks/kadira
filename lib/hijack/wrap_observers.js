wrapOplogObserveDriver = function(proto) {
  ...
  proto._publishNewResults = function(newResults, newBuffer) {
    ...
    newBuffer.forEach(function(item){
      data.push[item];
    });
    ...
    var docSize = Kadira.docSzCache.getSize(coll, query, opts, data);

    var count = newResults.size() + newBuffer.size();
    if(this._ownerInfo) {
      Kadira.models.pubsub.trackPolledDocuments(this._ownerInfo, count);
      Kadira.models.pubsub.trackDocSize(this._ownerInfo, docSize*data.length);
    } else {
      this._polledDocuments = count;
      this._docSize = {
        polledFetches: docSize*data.length
      }
    }
    ...
    proto[fnName] = function(a, b, c) {
      if(this._ownerInfo) {
        ...
        if(fnName === "_addPublished") {
          ...
          Kadira.models.pubsub.trackDocSize(this._ownerInfo, "liveFetches", docSize);
        }
      } else {
...
wrapPollingObserveDriver = function(proto) {
  ...
  proto._pollMongo = function() {
    ...
    // this now
    var count = this._results && this._results.size && this._results.size();
    count = count || 0;
      ...
      if(!ret._multiplexer.__kadiraTracked) {
        ...
        if(kadiraInfo.trace.type == 'sub') {
          ...
          _.each(observerDriver._docSize, function(count, key) {
            Kadira.models.pubsub.trackDocSize(ownerInfo, key, count);
          });
        }
      } else {
