...
['find', 'update', 'remove', 'insert', '_ensureIndex', '_dropIndex'].forEach(function(func) {
  ...
  mongoConnectionProto[func] = function(collName, selector, mod, options) {
    ...
    if(apmInfo) {
      var eventId = Kadira.tracer.event(apmInfo.trace, 'db', payload);
    }
    ...
    try{
      ...
      if(eventId) {
        Kadira.tracer.eventEnd(apmInfo.trace, eventId, endOptions);
      }
  ...
  ['forEach', 'map', 'fetch', 'count', 'observeChanges', 'observe', 'rewind'].forEach(function(type) {
    ...
    cursorProto[type] = function() {
      ...
      try{
        ...
        if(type == 'observeChanges' || type == 'observe') {
          ...
          if(ret._multiplexer) {
            ...
            if(apmInfo && apmInfo.trace.type == 'sub') {
              ...
              Kadira.models.pubsub.incrementHandleCount(apmInfo.trace, isCachedHandle);
            }
        ...
        if(eventId) {
          Kadira.tracer.eventEnd(apmInfo.trace, eventId, endData);
        }
        return ret;
      } catch(ex) {
        if(eventId) {
