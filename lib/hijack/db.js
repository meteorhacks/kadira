...
hijackDBOps = function hijackDBOps() {
  ...
  ['find', 'update', 'remove', 'insert', '_ensureIndex', '_dropIndex'].forEach(function(func) {
    ...
    mongoConnectionProto[func] = function(collName, selector, mod, options) {
      ...
      if(func == 'insert') {
        if(kadiraInfo) {
          var docSize = Kadira.docSzCache.getSize(collName, "", options, [selector]);

          if(kadiraInfo.trace.type === 'method') {
            Kadira.models.methods.trackDocSize(kadiraInfo.trace.name, "inserts", docSize);
          } else if(kadiraInfo.trace.type === 'sub') {
            Kadira.models.pubsub.trackDocSize(kadiraInfo.trace.name, "inserts", docSize);
          }
        }
      ...
      try{
        ...
        if(type == 'observeChanges' || type == 'observe') {
          ...
          if(type == 'fetch') {
            ...
            if(kadiraInfo) {
              if(kadiraInfo.trace.type === 'method') {
                Kadira.models.methods.trackDocSize(kadiraInfo.trace.name, "fetches", docSize);
              } else if(kadiraInfo.trace.type === 'sub') {
              ...
              Kadira.models.methods.trackDocSize("<not-a-method-or-a-pub>", "fetches", docSize);
            }
